...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    metric_logger.add_meter('loss_emd', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    for i, (image, caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_lm, loss_emd = model(image, caption, alpha = alpha)  

        loss = loss_emd + loss_lm

        loss.backward()
        optimizer.step()    

        metric_logger.update(loss_lm=loss_lm.item())
        metric_logger.update(loss_emd=loss_emd.item())
